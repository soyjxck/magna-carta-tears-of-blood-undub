...
The translator-facing workflow — the editable JSON schema, per-format length caps, engine tokens (`$n`, `$D<NN>`, `<...>` UI labels), the `celfid.lix` name/item catalog, and common validator errors — lives in [`TRANSLATING.md`](TRANSLATING.md), which is kept current with the pipeline. This section keeps only the format internals those catalogs are built from.

In brief: `python3 patch.py translate-extract` writes per-file catalogs to `translations/<ext>/<basename>.json`; edit only the `en` fields; `python3 patch.py translate-validate` checks caps/encoding; then `python3 patch.py build-iso --translations` folds the edits into the rebuild. Each catalog carries just text (`en`, plus `kr`/`jp` reference) — every structural field (offsets, lengths, caps, headers, trailers) is re-derived from USA bytes at build time, so there's nothing in the JSON to desync. Files without a catalog fall back to raw USA bytes, so partial coverage works.

Three format families back the catalogs:

### `.fpb` (PlayBook) — soft cap, per-record

One `records[]` array keyed by `seq`; each record carries its own `en` (+ `kr`/`jp` refs). On rebuild the data section is regenerated by concatenating each record's encoded `en` in order and computing `(seq, running_offset, length)` fresh — no diff/remap, no stored offsets. Strings grow freely (the data-section size header is rewritten). See [`.fpb` format](#fpb-playbook-format) for the on-disk layout and the implicit `seq=0` convention.

### Slot files (`.cht .odd .gft .cha .cdg .mdg .ecd .fds`) — hard cap

Fixed-stride tables: one null-terminated string per slot, with an optional engine trailer preserved verbatim. The per-slot byte budget is `tail_at - 1` (`tail_at` = where the trailer starts, or the stride when there's none), derived at build time. Per-extension geometry (`header_size + count × stride`, from `lib/translate/slot.py`):
...
Mixed binary/text: text sits at fixed offsets inside structure we leave untouched. The build copies the USA blob, then overwrites each text region with the encoded `en` null-padded to its cap (the run length before the next non-zero structural byte). No soft growth — these have hard structural sizes (e.g. `.pod` is fixed 12,328 B across all 148 files). Some detected "regions" are internal identifiers (asset paths, class names), not user-facing labels; `TRANSLATING.md` covers telling them apart.
...
SHIP.AFS hybrid:
  ...
  USA text overlays (18 exts):        288   (.cht .pod .tui .itm .odd .gft .cha
                                              .abi .sgi .cdg .mdg .dod .cls .ecd
                                              .att .nod .val .fds + .fpb blanket)
...
USA_TEXT_EXTS = (
    ...
    ".pod",  # Talisman tutorial popups (148 files, 12,328 B fixed)
    ".tui",  # UI labels
    ".itm",  # item names + descriptions
    ".odd",  # side-quest text + chest reward names
    ".gft",  # gift dialog
...
4. **`.btv`/`.lvt` voice trigger tables embed voice IDs as literal u32 values** (each `.btv` holds 54 × u32 voice IDs — see the SHIP taxonomy above). All 25 USA `.btv` files differ from source's because the IDs differ.
